<template>
        <div class="header-banner bg-banner-contact w-full h-[350px] bg-cover relative max-sm:hidden">
      <div class="absolute bottom-0 left-0 right-0 text-center">
        <h1 class="bg-white inline-block text-purple-fonce text-5xl py-6 px-24 rounded-t-2xl font-bold font-cgothic max-sm:px-8 max-sm:text-4xl">Contact - Prévention Prostitution Normandie</h1>
      </div>
      </div>

    <div class="header-banner bg-banner-contact-mobile w-full h-[350px] bg-cover relative hidden max-sm:block">
      <div class="absolute bottom-0 left-0 right-0 text-center">
        <h1 class="bg-white inline-block text-purple-fonce text-5xl py-6 px-24 rounded-t-2xl font-bold font-cgothic max-sm:px-8 max-sm:text-4xl" aria-label="Page de contact">CONTACT</h1>
      </div>
      </div>

<!-- Formulaire de contact -->
<div class="contact-container flex flex-wrap justify-center items-center gap-10 mt-20 font-jost max-sm:mt-10 mb-8">
    ...
    <form @submit.prevent="sendForm" class="flex flex-col gap-6">
      <div class="first-row flex max-md:gap-8 w-full gap-8 max-sm:flex-col">
        <input v-model="form.name" type="text" placeholder="Nom" class="border-b border-purple-fonce text-xl w-full max-md:w-auto pb-4" required>
        <input v-model="form.email" type="email" placeholder="Email" class="border-b border-purple-fonce text-xl w-full max-md:w-auto pb-4" required>
      </div>
      <div class="second-row flex max-md:gap-8 w-full gap-8 max-sm:flex-col">
        <input v-model="form.subject" type="text" placeholder="Objet" class="border-b border-purple-fonce text-xl w-full max-md:w-auto pb-4" required>
        <input v-model="form.phone" type="tel" placeholder="Téléphone" class="border-b border-purple-fonce text-xl w-full max-md:w-auto pb-4">
      </div>
      ...
      <div class="submit flex justify-end w-full max-sm:justify-center">
